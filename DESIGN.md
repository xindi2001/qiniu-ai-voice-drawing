...
| 命令 | 示例 | 状态 | 说明 |
|------|------|------|------|
| 修改颜色 | 「把圆改成绿色」 | ⏳ 引擎已支持，需 LLM 解析 targetId | 需图形选中/引用机制 |
| 删除图形 | 「删除最后一个圆」 | ⏳ 引擎已支持，需 targetId | 需图形索引或语义引用 |
| 移动图形 | 「把圆移到左边」 | ❌ 未实现 | 需坐标语义理解 |
| 语音输入 | 麦克风说话 | ⏳ 占位 | `useSpeechRecognition` 待接入 |
...
| 功能 | 原因 | 计划 |
|------|------|------|
| 完整语音识别 | MVP 聚焦文本链路 | Day 2 接入 Web Speech API |
| 图形语义引用（「那个圆」） | 需维护图形上下文传给 LLM | Day 2 传 shapes 列表给 prompt |
...
- [ ] 接入 Web Speech API
- [ ] 完善 modify/delete 的 LLM prompt（含图形列表上下文）
- [ ] 错误提示与加载状态优化
- [ ] 更多颜色/位置语义
