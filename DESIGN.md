...
| 移动图形 | 「把圆移到左边」 | ✅ DeepSeek；Mock 部分 | LLM 支持位置；Mock 支持左右上下偏移 |
| 语音输入 | 麦克风说话 | ✅ 已实现 | 阿里云 ASR 优先，Web Speech API 降级 |
| TTS 播报 | speak 字段 | ✅ 已实现 | Web Speech Synthesis |
| 同音字纠正 | ASR 误识别修正 | ✅ 已实现 | 后端 `AsrHomophoneHelper` |
| 确认后再执行 | 识别后手动提交 | ✅ 已实现 | VoicePanel 可选开关 |
...
| 功能 | 原因 | 计划 |
|------|------|------|
| 完整语音识别 | MVP 聚焦文本链路 | ✅ Day 2 Web Speech；Day 3 阿里云 ASR |
| 图形语义引用（「那个圆」） | 需维护图形上下文传给 LLM | ✅ Day 2 传 sceneContext |
| ASR 同音字纠正 | 语音识别常见误字 | ✅ Day 3 `AsrHomophoneHelper` |
| 确认后再执行 | 避免误识别直接绘图 | ✅ Day 3 VoicePanel 开关 |
| 后端持久化 | 无状态绘图，MVP 不要求 | 不计划实现 |
| 用户认证 | 单用户演示场景 | 不计划实现 |
| 生产后端部署 | 当前仅本地运行 | Day 3 待完成（可选 Railway/Render） |
| GitHub Pages | 前端静态托管 | Day 3 待完成 |
| 演示视频 | 功能展示录制 | Day 3 待完成 |
...
- [x] 阿里云 ASR 集成（Token 获取、转写接口、前端录音上传）
- [x] DeepSeek LLM 端到端联调
- [x] 403 / 代理问题排查与文档
- [x] ASR 同音字纠正
- [x] 「确认后再执行」交互
- [x] README / DESIGN 里程碑同步
...
7. 「删除最后一个」          → 删除图形
8. 「把圆移到左边」          → 修改位置（DeepSeek 或 Mock）
9. 点击麦克风语音输入        → 阿里云 ASR 或 Web Speech 识别并执行
10. （可选）勾选「确认后再执行」→ 核对识别文字后点「确认执行」
```
